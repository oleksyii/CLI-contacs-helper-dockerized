...
class Birthday(Field):
    ...
    def to_dict(self):
        return {
            "value": date_to_string(self.value) if self.value else ""
        }  # assuming this is your date format
...
class Record:
    ...
    def edit_phone(self, phone_to_edit: str, new_phone: str):
        ...
        if old_phone in self.phones:
            ...
            raise ValueError(
                f"No such phone number is present in a current record.\n The record is: {str(self)}"
            )
        ...
        for user in self.data.values():
            ...
            if 0 <= (birthday_this_year - today).days <= days:
                ...
                upcoming_birthdays.append(
                    Record(
                        user.name.value,
                        [str(phone) for phone in user.phones],
        ...
        return [
            {
                name: {
                    ...
                    "birthday": str(self.data[name].birthday),
                }
